...
exports.genre_delete_post = function (req, res, next) {
  ...
  async.parallel({
    ...
    if (err) { return next(err); }
    //Success
    if (results.genre_books.length > 1) {
      //Genre has books, so render in same way as for GET route
      res.render('genre_delete', { title: 'Delete Genre', genre: results.genre, genre_books: results.genre_books });
      return;
    } else {
      //Genre has no books, delete object and redirect to genre list page
...
exports.genre_update_post = [
  ...
  //Validate that the name field ain't empty
  ...
  (req, res, next) => {
    ...
    //Create a genre object with escaped & trimmed data, (and the old id!)
    var genre = new Genre({
      name: req.body.name,
      _id: req.params.id
    });
    if (!errors.isEmpty()) {
      // No errors, render the form again with sanitized values/ error messages
      ...
      Genre.findByIdAndUpdate(req.params.id, genre, {}, function (err, thegenre) {
        if(err) { return next(err) }
        // Successful, redirect to the genre detail
